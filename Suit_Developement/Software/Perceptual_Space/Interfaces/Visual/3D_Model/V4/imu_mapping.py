...
def convert_quaternion(body, w, x, y, z):
    ...
    That delta now arrives encoded as the Euler triplet
    heading/pitch/roll and is rebuilt by quaternion.from_euler
    before reaching this function. The convention is unchanged
    (the firmware derives those angles from that same local
    delta), so the correction below still applies verbatim.
