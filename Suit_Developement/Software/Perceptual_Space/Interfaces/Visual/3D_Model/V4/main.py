...
from network import AsyncMQTTClient
...
BASE_CAPTION = "ESP32 Motion Capture 3D"
...
def run_opengl():
    ...
    This function initializes the OpenGL window, subscribes to
    the MQTT topic the bridge publishes the suit data on,
    manages the user interface events, updates the camera,
    acquires IMU data, updates the skeleton, records the data
    when requested and renders the complete scene until the
    application exits.
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_c:
                    # Calibrate only on fresh data: re-zeroing on a
                    # stale packet would store an outdated pose as
                    # the reference.
                    latest, _, cal_age = client.get()
                    if latest is None or cal_age >= STALE_AFTER:
                        print("Calibration skipped: no fresh data")
                    elif calibrate(latest):
                        skeleton.resync()
        ...
        if fresh:
            ...
            status = f"LIVE ({system})"
        elif data is None:
            status = "DISCONNECTED"
        else:
            status = "STALE - holding last pose"
