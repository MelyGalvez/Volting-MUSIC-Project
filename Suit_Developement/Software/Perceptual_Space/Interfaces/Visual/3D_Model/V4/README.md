...
├── imu_log.csv             ← reference capture; the recorder follows its layout
├── capture_*.csv           ← recorded sessions, one file per `R` toggle
...
file. The layout follows `imu_log.csv`, so a recording drops straight into
whatever already reads those logs.

**Responsibilities.** Open a timestamped CSV file, write one row per distinct
frame with the whole body encoded as JSON in the last column, and close it
cleanly.

**Main class — `Recorder`.**

- `__init__()` — creates an inactive recorder with its file handle, writer,
  lock, `_last_seq`, `_last_flush` and `_frame_count`.
- `start()` — builds the filename `capture_%Y%m%d_%H%M%S.csv` from the local
  ...
  writes the header row `datetime, epoch, arduino_timestamp, system, json`,
  and enables recording.
- `add(data)` — ignores anything when not recording, and ignores non-dicts,
  packets without a list `imu_data`, and duplicates of the last `seq`. It
  builds one JSON document holding `seq`, `timestamp`, `system` and every
  segment with its orientation, then writes a single row: host wall-clock,
  epoch, firmware timestamp, state and that document. Fields are read with
  `.get(...)` so one missing from the packet is recorded as `null` rather than
  raising. It flushes at most once per `RECORDER_FLUSH_PERIOD_S`.
...
> The recorded fields deliberately mirror the wire format, including the `ok`
> and `cal` flags. A recording therefore preserves the distinction between a
> T-pose-relative reading and an absolute one, which a file of processed angles
> would have thrown away. Keeping them inside one `json` column is what makes a
> row an atomic frame rather than a single joint.
...
when enabled — the CSV capture file.
...
flowchart TD
    ...
    F -.-> U["recorder.add(): one CSV row per frame, whole body as JSON"]
```
...
detects reboots and is recorded in the capture; `system` gates whether it may
...
**What.** A file named `capture_YYYYMMDD_HHMMSS.csv`, created in the working
directory and laid out like `imu_log.csv`:

| Column | Meaning |
|---|---|
| `datetime` | Host wall-clock, `YYYY-MM-DD HH:MM:SS.mmm` |
| `epoch` | The same instant as a Unix timestamp |
| `arduino_timestamp` | Firmware uptime in ms, straight from the packet |
| `system` | Firmware state for that frame |
| `json` | The whole body for that frame, as one JSON document |

One row is therefore one complete pose. The `json` column holds:

```json
{"seq":9768,"timestamp":493292,"system":"ready","imu_data":[
 {"body":"back_upper","ok":true,"cal":true,"heading":-1.79,"pitch":-32.53,"roll":-21.71},
 {"body":"back_lower","ok":true,"cal":true,"heading":5.85,"pitch":28.78,"roll":31.28}]}
```

**Why a JSON column rather than one row per sensor.** A row then describes the
whole body at one instant instead of a single joint: frames stay atomic, and
the list of segments can change without touching the CSV header. The file is
still a plain CSV that any spreadsheet opens, and `json.loads()` on the last
column gives the pose back.

**Why it is flushed at most once per second** rather than after every frame: a
flush is a system call, and the render thread is the one making it.
...
identical sample into the filters, the recorder to keep the capture reflecting
the true sensor rate rather than the render rate.
...
| Constant | Value | Meaning |
|---|---|---|
| `RECORDER_FLUSH_PERIOD_S` | `1.0` | Minimum seconds between capture flushes, to keep syscalls off the render thread |
...
| `timestamp` | Firmware uptime in ms | Detects reboots; written to the capture |
