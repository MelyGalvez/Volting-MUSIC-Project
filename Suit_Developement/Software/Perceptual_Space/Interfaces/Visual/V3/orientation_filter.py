...
# ------------------ Constructor ------------------
    ...
    def push(self, q):
        ...
        are rejected, with one refinement: a jump CONFIRMED by
        the next measurement (two consecutive raw samples that
        agree with each other) is accepted immediately. Real
        motion is continuous, so a genuine fast move confirms
        itself after one packet, while an isolated glitch never
        does. Without this, fast intentional moves lagged up to
        ...
        if change > allowed and self._rejects < self.max_rejects:
            consistent = (
                ...
                and angle_between(q, self._last_raw) <= allowed
            )
